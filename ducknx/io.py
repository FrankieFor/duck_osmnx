...
def load_graphml(
    ...
    *,
    ...
    """
    Load a ducknx-saved GraphML file from disk or GraphML string.
    ...
    `bool`, consider using the built-in `dx.io._convert_bool_string` function
    to properly handle "True"/"False" string literals as True/False booleans:
    `dx.load_graphml(fp, node_dtypes={my_attr: dx.io._convert_bool_string})`.
...
def save_graph_xml(
    ...
    *,
    ...
    graphs for subsequent ducknx analysis. To save/load graphs to/from disk for
    later use in ducknx, use the `io.save_graphml` and `io.load_graphml`
    functions instead.
    ...
    `G` should also have been created with `dx.settings.all_oneway=True` for
